...
public class MyBigView extends View implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener,
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mImageWidth <= mImageHeight) {
            ...
            isHorizontal = true;
            mScale = mViewHeight / mImageHeight;
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (isTaped) {
            if (isHorizontal) {
                mRect.offset((int) distanceX * 2, (int) distanceY);
            } else {
                mRect.offset((int) distanceX / 3, (int) distanceY / 3);
            }

        } else {
            if (isHorizontal) {
                mRect.offset((int) distanceX * 3, (int) distanceY);
            } else {
    ...
    public boolean onDoubleTap(MotionEvent e) {
        ...
        if (mCurrentScale > mScale) {
            ...
            mRect.top = y - (int) (mViewHeight / mCurrentScale) / 2;
            mRect.left = x - (int) (mViewWidth / mCurrentScale) / 2;
            mRect.right = x + (int) (mViewWidth / mCurrentScale) / 2;
            mRect.bottom = y + (int) (mViewHeight / mCurrentScale) / 2;
        }
